...
@section('konten')
    ...
    <div class="container-fluid p-4">
        ...
        <form method="POST" id="formDataPelanggan" action="{{ route('suntingeditorders', $nomer_order) }}">

            @csrf
            {{-- head req edit --}}
            {{-- @dd($request) --}}
            <h1>Dari Order Number {{ $nomer_order }}</h1>
            <input type="hidden" value="{{ $nomer_order }}" id="order_number" name="order_number">
            {{-- body req edit --}}
            <div class="body-reqEdit flex-column p-4 ">
                ...
                <div class="d-flex flex-row justify-content-between mb-4 gap-5" id="form-juragan">
                    ...
                    <div class="col">
                        ...
                        <b><p id='dilayani'></p></b>
                        <input type="hidden" value="" name="method" id="lmethod">
                        <input type="hidden" value="" name="status" id="lstatus">
                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body m-3 py-0">
                    ...
                    <form method="POST" id="formongkir" action="{{ url('super-admin/request/ongkir') }}">
                        @csrf
                        <div class="mb-4 ">
    ...
    <div class="modal fade" id="Modalbiayalain" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body m-3 py-0">
                    <form method="POST" id="formbiayalain" action="{{ url('super-admin/request/lain') }}">
                        @csrf
                        <div class="mb-3 ">
                            <input type="hidden" value="{{ $nomer_order }}" name="order_number">
    ...
    <div class="modal fade " id="addOrder" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <form method="POST" id="tambahForm" action="{{ url('super-admin/request/keranjang') }}">
                    @csrf
        ...
        <div class="modal fade " id="editOrder{{ $isi->id }}" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    ...
                    <form method="POST" action="{{ url('super-admin/request/editkeranjang/' . $isi->id) }}">
                        @csrf
                        <div class="modal-body m-3 py-0">
                            <div class="d-flex flex-row justify-content-between gap-4  mb-4">
                                <div class="col">
...
@section('js')
    <script>
        ...
        function tampil(){
            ...
            const orderdate  = localStorage.getItem('sordedate_' + order_number);
            document.getElementById("itanggal").value=orderdate;
            document.getElementById("tanggal").value=orderdate;

            const juragan  = localStorage.getItem('sidjuragan_' + order_number);
            document.getElementById("ijuragan").value=juragan;

            const namejuragan  = localStorage.getItem('snamejuragan_' + order_number);
            document.querySelector(".pjuragan").innerText=namejuragan;

            const source  = localStorage.getItem('ssource_' + order_number);
            document.getElementById("source").innerText=source;
            document.getElementById("isource").value=source;

            const dilayani  = localStorage.getItem('snameservedby_' + order_number);
            document.querySelector("#dilayani").innerText=dilayani;

            const iddilayani  = localStorage.getItem('sidservedby_' + order_number);
            document.querySelector("#iddilayani").value=iddilayani;

            const csname  = localStorage.getItem('snamecs_' + order_number);
            document.querySelector("#pelanggan").value=csname;

            const csid  = localStorage.getItem('sidcs_' + order_number);
            document.querySelector("#idpelanggan").value=csid;

            const notes  = localStorage.getItem('snote_' + order_number);
            document.querySelector("#note").value=notes;

            const method  = localStorage.getItem('smethod_' + order_number);
            document.querySelector("#lmethod").value=method;

            const status  = localStorage.getItem('sstatus_' + order_number);
            document.querySelector("#lstatus").value=status;
    ...
    <script>
        ...
        function saveOrder() {
            var id_akun = document.querySelector('#order_number').value;
                localStorage.removeItem('rorderdate_' + id_akun);
                localStorage.removeItem('sidjuragan_' + id_akun);
                localStorage.removeItem('snamejuragan_' + id_akun);
                localStorage.removeItem('ssource_' + id_akun);
                localStorage.removeItem('snameservedby_' + id_akun);
                localStorage.removeItem('sidservedby_' + id_akun);
                localStorage.removeItem('sordedate_' + id_akun);
                localStorage.removeItem('snamecs_' + id_akun);
                localStorage.removeItem('sidcs_' + id_akun);
                localStorage.removeItem('snote_' + id_akun);
                localStorage.removeItem('smethod_' + id_akun);
                localStorage.removeItem('sstatus_' + id_akun);

                ...
                <span class="d-flex align-items-center gap_custom">
                    ...
                    @if ($status->contains('id_status', 3))
                        @php
                            ...
                            $text = $status3->where('kelengkapan', 'Lengkap')->isNotEmpty() ? 'Data Lengkap' : 'Data Belum Lengkap';
                        @endphp
                        <svg width="36" height="35" viewBox="0 0 22 31" fill="none" data-bs-toggle="tooltip"
                            data-bs-placement="bottom" data-bs-title="{{ $text }}"
                            ...
                            <g id="Icon">
                                <path id="Vector"
                                    ...
                                    fill="{{ $warna }}" />
                                <path id="Vector 1480" d="M5.77344 15.0771H15.9273" stroke="{{ $warna }}"
                                    stroke-width="1.23077" stroke-linecap="round" />
                                <path id="Vector 1481" d="M5.77344 18.1533H15.9273" stroke="{{ $warna }}"
                                    stroke-width="1.23077" stroke-linecap="round" />
                                <path id="Vector 1482" d="M5.77344 21.2305H15.9273" stroke="{{ $warna }}"
                                    stroke-width="1.23077" stroke-linecap="round" />
                            </g>
                        </svg>
                    @else
                            ...
                            <svg width="29" height="9" viewBox="0 0 29 9" fill="none"
                                ...
                                <g id="Line">
                                    <circle id="Ellipse 1852" cx="23.9952" cy="4.69051" r="4.30769"
                                        fill="#828282" />
                                    <line id="Line 32" y1="4.69111" x2="27.6923" y2="4.69111"
                                        stroke="#828282" stroke-width="2.46154" />
                                </g>
                            </svg>
                        </span>
                    @else
                ...
                <div class="d-flex justify-content-between  gap-1" style="width: max-content !important;">
                    @if ($status->contains('id_status', 1))
                        ...
                        <p data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="pesanan ditambahkan"
                            ...
                            <b>
                                {{ \Carbon\Carbon::parse($status1->updated_at)->format('Y/m/d') }}
                            </b>
                        </p>
                    @else
                    @endif
                    @if ($status->contains('id_status', 2))
                        ...
                        @endphp
                        <p data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="{{ $titleText }}"
                            data-bs-custom-class="custom-tooltip" style="font-size:45%;width:43px;color:{{ $fillColor }};"
                            class=" m-0 justify-content-center d-flex ">
                            <b>
                                {{ \Carbon\Carbon::parse($status2->updated_at)->format('Y/m/d') }}
                            </b>
                        </p>
                    @else
                    @endif
                    @if ($status->contains('id_status', 3))
                        @php
                            ...
                            if ($status3) {
                                $warna = $status3->kelengkapan === 'Lengkap' ? '#24A240' : '#828282';
                                $text = $status3->kelengkapan === 'Lengkap' ? 'Data Lengkap' : 'Data Belum Lengkap';
                            }
                        @endphp
                        @if (!empty($status3))
                            <p data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="{{ $text }}"
                                data-bs-custom-class="custom-tooltip" style="font-size:45%;width:43px;color:{{ $warna }};"
                                class=" m-0 justify-content-center d-flex ">
                                <b>
                                    {{ \Carbon\Carbon::parse($status3->updated_at)->format('Y/m/d') }}
                                </b>
                            </p>
                        @endif
                    @endif
                    ...
                    @if ($status->contains('id_status', 4))
                        @php
                            ...
                            if ($status9) {
                                $warna = $status9->kelengkapan === 'selesai' ? '#24A240' : '#828282';
                                $text = $status9->kelengkapan === 'selesai' ? 'Sudah Dipacking' : 'Belum Dipacking';
                            }
                        @endphp
                        @if (!empty($status9))
                            <p data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="{{ $text }}"
                                ...
                                class=" m-0 justify-content-center d-flex ">
                                <b>
                                    {{ \Carbon\Carbon::parse($status9->updated_at)->format('Y/m/d') }}
                                </b>
                            </p>
                        @endif
                    @endif
                    @if ($status->contains('id_status', 5))
                            <p data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Diantar"
                                ...
                                class=" m-0 justify-content-center d-flex ">
                                <b>
                                    {{ \Carbon\Carbon::parse($status10->updated_at)->format('Y/m/d') }}
                                </b>
                            </p>
                    @endif
                </div>

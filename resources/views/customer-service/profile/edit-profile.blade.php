...
@section('konten')
    <section class="container-edit_profile rounded">
        ...
        <article class="header-edit_profile px-lg-4 px-md-4">
            <!-- <a href="/cs/profile" class="back-profile">
                <i class="fa-solid fa-arrow-left icon_back-profile"></i>
            </a> -->
            <h1 class="text-white fs-2 fw-bold m-0 py-5">{{ $title }}</h1>
        </article>
        {{-- Akhir Header Profile --}}

        <form action="{{ route('cs.profile.update') }}" method="POST" class="bg-white pt-5 body-edit_profile d-flex justify-center align-items-center">
            @method('PUT')
            @csrf
            <div
                ...
                <div class="row d-flex justify-content-between p-0 my-3">
                    <div class="col-lg-6 col-md-12 d-flex flex-column gap-1 mb-md-3">
                        <label class="text-secondary fw-semibold m-0">Nama<br /></label>
            ...
            <div class="d-flex gap-5 btn-edit_save my-5">
                <button class="btn btn_batal-edit" type="button" onclick="window.location.href='{{ route('cs.profile') }}'">Batal</button>
                <button class="btn btn_simpan-edit" type="submit">Simpan</button>
            </div>
        </form>
    </section>
